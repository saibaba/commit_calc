module.exports = function () {
    ...
    function performSmoothing(monthlyCharges, rv) {
        var daysInYear  = { 'Jan' : 31, 'Feb': 28, 'Feb-Leap': 29, 'Mar': 31, 'Apr' : 30, 'May' : 31, 'Jun': 30, 'Jul': 31,
                            'Aug' : 31, 'Sep': 30, 'Oct' : 31, 'Nov': 30, 'Dec': 31 };
        var factor = 30.4;
        var p, d, smoothedMonthlyCharges;
        var perMonthCharges = {};
 
        for (p in daysInYear) {
            if (daysInYear.hasOwnProperty(p) ) {
                d = daysInYear[p];
                smoothedMonthlyCharges = roundNumber(monthlyCharges * d / factor);
                perMonthCharges[p] = smoothedMonthlyCharges; 
            }
        }
        rv['smoothedCharges']  = perMonthCharges; 
    }

    function calcDiscount(amount, term, prepayFlag, smoothing) {
        ...
        if (smoothing) performSmoothing(monthlyCharges, rv);
        return rv;
    }
